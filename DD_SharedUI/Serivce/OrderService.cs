    ...
    public class OrderService : IOrderService
    {
        ...
        public async Task<OrderDTO> Create(StripePaymentDTO paymentDTO)
        {
            ...
            if (response.IsSuccessStatusCode)
            {
                ...
                var response1 = await _httpClient.PostAsync($"/api/account/UpdateUserAddress", bodyContent1);
        ...
        {
            var response = await _httpClient.GetAsync($"/api/order/Get/{orderHeaderId}");
        ...
        {
            var response = await _httpClient.GetAsync($"/api/Order/GetAll/{userId}");
            if (response.IsSuccessStatusCode)
            {
                var content = await response.Content.ReadAsStringAsync();
        ...
        public async Task<bool> UpdateOrderStatus(int orderId, string status)
        {
            ...
            var bodyContent = new StringContent(content, Encoding.UTF8, "application/json");
            var response = await _httpClient.PutAsync($"/api/order/updatestatus", bodyContent);
            string responseResult = response.Content.ReadAsStringAsync().Result;
            if (response.IsSuccessStatusCode)
            {
        ...
        public async Task<bool> NotifyOrderChecked(int orderId)
        {
            ...
            var bodyContent = new StringContent(content, Encoding.UTF8, "application/json");
            var response = await _httpClient.PutAsync($"/api/order/notifyOrderChecked", bodyContent);
            string responseResult = response.Content.ReadAsStringAsync().Result;
            if (response.IsSuccessStatusCode)
            {
